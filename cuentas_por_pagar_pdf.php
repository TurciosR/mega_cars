...
class PDF extends FPDF
{
    ...
    public function Header()
    {
        ...
        $this->Cell(195,6,utf8_decode("CUENTAS POR PAGAR"),0,1,'C');
        // Salto de línea
        $this->Ln(5);
        $set_y=$this->GetY();
        $set_x=$this->GetX();
        ...
        $this->Cell(65,5,"DESCRIPCION",1,0,'C',0);
        $this->Cell(65,5,"FACTURAS PENDIENTES",1,0,'C',0);
        $this->Cell(65,5,"DEUDA TOTAL",1,1,'C',0);
...
while($row=_fetch_array($result1)){
  ...
  $array_data = array(
    array(utf8_decode($proveedor),65,"L"),
    array(utf8_decode($facp),65,"C"),
    array(utf8_decode(number_format($deuda,2)),65,"R"),
  );
...
$array_data = array(
  array(utf8_decode("TOTALES"),130,"C"),
  array(utf8_decode(number_format($deuda_total,2)),65,"R"),
);
$pdf->LineWriteB($array_data);

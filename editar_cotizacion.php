...
function initial()
{
  ...
  $sql_cot = _query("SELECT id_cliente, fecha, vigencia FROM cotizacion WHERE id_cotizacion='$id_cotizacion'");
                        ...
                        <div class="form-group has-info">
                          ...
                          <select class="form-control select" name="id_cliente" id="id_cliente">
                            ...
                            while($row_cli = _fetch_array($sqlcli))
                            {
                              ...
                              if($id_cliente == $row_cli["id_cliente"])
                              {
                                echo " selected ";
                              }
                        ...
                        <div class="table100 ver1 m-b-10">
                          ...
                          <div class="table100-body js-pscroll">
                            <table>
                              <tbody id="inventable">
                              <?php
                              $sql = _query("SELECT p.id_categoria, p.id_producto, p.descripcion, cd.precio_venta, cd.cantidad, cd.id_presentacion, cd.subtotal FROM cotizacion_detalle as cd JOIN producto as p ON cd.id_prod_serv=p.id_producto WHERE id_cotizacion='$id_cotizacion'");
                              $filas = 0;
...
function insertar()
{
  ...
  if ($cuantos>0)
  {
    ...
    if ($nrows_fact==0)
    {
      ...
      _delete("cotizacion_detalle" ,$where);
    }
    ...
    else
    {
      ...
      if($id_fact == 0)
      {
        ...
        $xdatos['msg']="Ya se registro una Cotización con estos detalles";
      }
      else
      {
        $xdatos['typeinfo']='Error';
        $xdatos['msg']='Cotización no pudo ser registrada!'.$insertar_fact."-".$insertar_fact_dett;
      }
    }
  }

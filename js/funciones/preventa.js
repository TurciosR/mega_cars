                ...
                $.ajax({
                    ...
                    success: function(datax) {
                        ...
                        if (datax.typeinfo == "Success") {
                            //$("#btnDetalleTrabajo").attr('disabled', true);
                            ...
                            totales();
                            cargar_detalles(datax.id_factura)
                        } else {
...
function cargar_detalles(value_factura) {

    $.ajax({
        type: "POST",
        url: "agregar_detalles_trabajo.php",
        data: {
            id_factura: value_factura,
            process: 'loadData'
        },
        dataType: "json",
        success: function (datax) {
            if (datax.typeinfo == "Success") {
                //$("#btnDetalleTrabajo").attr('disabled', true);
                $('#viewModal3').on('shown.bs.modal', function (e) {
                    $("#placa_moto").attr('disabled', true);
                    $(".id_empleado").attr('disabled', true);
                    $("#diagnostico_inicial").attr('disabled', true);
                    $('#btnSave').attr('disabled', true);

                    $("#placa_moto").val(datax.placa_moto)
                    $("#diagnostico_inicial").val(datax.diagnostico_inicial)
                    $("#fecha_ingreso").val(datax.fecha_ingreso)
                    $(".id_empleado").val(datax.id_empleado);
                    $(".id_empleado").change();
                });
                ...
                elemento.placa_moto = datax.placa_moto;
                elemento.id_empleado = datax.id_empleado;
                elemento.diagnostico_inicial = datax.diagnostico_inicial;
                elemento.fecha_ingreso = datax.fecha_ingreso;

                arrayDetalle.push({...elemento});

                localStorage.setItem("detalle_trabajo", JSON.stringify(arrayDetalle));
            }
        }
    });
}
...
function totales() {
    ...
    if (tipo_impresion == "CCF") {
        $("#inventable tr").each(function() {
            ...
            precioa = $(this).find('#precio_venta').val();
            precioa = parseFloat(precioa);
            precioa = round(precioa, 4);

            total_definitivo += subt(subt_cant, precioa);

            if (ex == 0) {
                subt_gravado = subt(subt_cant, precioa);
                subt_gravado = parseFloat(subt_gravado);
                subt_gravado = subt_gravado / (1 + parseFloat(iva));
            } else {
                subt_exento = subt(subt_cant, precioa);
            }
        ...
        swal({
            ...
            if (isConfirm) {
                $.ajax({
                    ...
                    success: function(datax) {
                        if (datax.typeinfo == "Success") {
                            eliminar_detalle_trabajo(id_factura)
                            display_notify(datax.typeinfo, datax.msg);
...
function guardar_preventa() {
    ...
    $("#inventable tr").each(function(index) {
        ...
        var description = "";
        if(id==-9999 && id_presentacion==-9999)
        {
          description = $(this).find(".descripcion_insumo").text();
        }
    ...
    if(!localStorage.getItem("detalle_trabajo")){
        msg = 'Por favor ingrese los detalles de este trabajo';
        sel_vendedor = 0;
    }
...
function guardar_detalle_trabajo(){
    ...
    $.ajax({
        ...
        url: "agregar_detalles_trabajo.php",
        data: {
            placa_moto: detalles_trabajo.placa_moto,
            id_empleado: detalles_trabajo.id_empleado,
            diagnostico_inicial: detalles_trabajo.diagnostico_inicial,
            fecha_ingreso: detalles_trabajo.fecha_ingreso,
            process: 'insert'
        },
        dataType: "json",
        success: function (datax) {
...
function eliminar_detalle_trabajo(value_factura){
